...
ColorPickerController::ColorPickerController()
  : active(false),
    ...
    settings(QDir(qApp->applicationDirPath() + "/config").absoluteFilePath("ColorPickerPlugin.ini"), QSettings::IniFormat),
    redSetting("Red"),
    greenSetting("Green"),
    blueSetting("Blue")
{
  qDebug() << "ColorPickerPlugin: Loading settings from: " << settings.fileName();
...
void ColorPickerController::activate(std::shared_ptr<ILight> pLight)
{
  if(!active)
  {
    light = pLight;
